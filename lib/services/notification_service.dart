...
class NotificationService {
  ...
  Future<bool> initialize() async {
    try {
      ...
      _subscription = NotificationsListener.notificationStream!.listen(_onNotification);
  ...
  Future<bool> requestNotificationPermission() async {
    try {
      bool enabled = await platform.invokeMethod('isNotificationListenerEnabled');
  ...
  void _onNotification(NotificationEvent event) {
    try {
      ...
      if (monitoredPackages.contains(event.packageName)) {
        ...
        _parser.parseNotificationTransaction(
          event.title ?? '',
          event.text ?? '',
          event.packageName ?? '',
          DateTime.now(),
